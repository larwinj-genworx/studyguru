...
class WorkedExampleAgent(BaseStructuredAgent):
    ...
    def execute(
        ...
        *,
        ...
        prompt = (
            "You are the worked-example generation stage in a production learning-content pipeline.\n"
            "Your job is to create solved examples that help students understand how the concept is actually used, reasoned through, and completed step by step.\n"
            "The examples must be accurate, easy to follow, and educational from a student's point of view.\n\n"
            ...
            f"Evidence Pack:\n{evidence_text}\n\n"
            "Primary goals:\n"
            "1. Help students see how the concept works in practice.\n"
            "2. Make each worked example understandable at the stated grade level.\n"
            "3. Show reasoning clearly enough that a student can imitate the process later.\n"
            "4. Progress from easier examples to more demanding ones without becoming confusing.\n\n"
            "Return JSON with key: examples (list).\n"
            "Each example must be an object with keys: title, prompt, steps, result, example_type.\n\n"
            "Required structure:\n"
            "- Include 4 to 6 worked examples.\n"
            "- Difficulty should increase gradually across the set.\n"
            "- Each example must stay strictly inside this concept only.\n"
            "- Each example must contain 3 to 6 short, meaningful steps.\n"
            "- Steps must show reasoning, not just final operations.\n"
            "- The final result must be clear, correct, and consistent with the worked steps.\n\n"
            "Student-understanding rules:\n"
            "- Write as if the student is learning how to solve or interpret this for the first time.\n"
            "- Avoid jumps in logic.\n"
            "- Avoid over-compressing the solution into expert shorthand.\n"
            "- Use clear intermediate reasoning when choosing a formula, applying a rule, or making a transformation.\n"
            "- If units, sign conventions, or symbolic meaning matter, make them visible in the worked steps.\n"
            "- Prefer examples that illuminate the concept, not decorative or unrealistic examples.\n\n"
            "Accuracy rules:\n"
            "- Base the examples on the evidence pack and keep formulas, units, terminology, and reasoning consistent.\n"
            "- Do not invent unsupported facts, values, or domain assumptions.\n"
            "- If the concept is quantitative, examples should show correct selection of relation, substitution, simplification, and verification.\n"
            "- If the concept is derivational, examples should justify each symbolic transformation clearly.\n"
            "- If the concept is scenario-based, examples should still include a proper worked solution, not only a description.\n\n"
            "Quality rules for steps:\n"
            "- Each step should represent one meaningful move in the solution.\n"
            "- Steps should be written in plain student-friendly language.\n"
            "- Avoid vague steps like 'solve it' or 'do the calculation'.\n"
            "- Avoid repeating the same step in different words.\n"
            "- Do not skip from setup directly to answer.\n\n"
            f"{self._style_instruction(preferred_style)}\n"
            "Formatting rules:\n"
            "- Use plain text equations only.\n"
            "- Do not use markdown tables, LaTeX, or HTML.\n"
            "- Keep titles concise and useful.\n"
            "- Keep prompts realistic, concept-specific, and study-relevant.\n"
